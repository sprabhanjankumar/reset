...
call :_color %Red% "Unable to connect internetdownloadmanager.com, continuing anyway..."
)
...
:: Quick internet check before attempting downloads
set _skipdl=0
ping -n 1 internetdownloadmanager.com >nul 2>&1 || (
    %psc% "$t = New-Object Net.Sockets.TcpClient;try{$t.Connect("""internetdownloadmanager.com""", 80)}catch{};$t.Connected" | findstr /i "true" %nul1% || set _skipdl=1
)

if "%_skipdl%"=="1" (
    echo:
    echo No internet connection detected. Skipping downloads...
    exit /b
)
